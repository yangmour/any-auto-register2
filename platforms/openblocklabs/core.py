...
def _build_multipart(fields: list, boundary: str = "----WebKitFormBoundaryPyAPI") -> tuple:
    body = ""
...
def _make_signals() -> str:
    ...
    data = {
        ...
        "screen": {
            ...
            "availWidth": 1470, "availHeight": 956,
            "windowOuterWidth": 1470, "windowOuterHeight": 956,
...
class OpenBlockLabsRegister:
    ...
    def _post_action(self, url: str, fields: list, router_state: str):
        ...
        return self.s.post(url, data=body, headers={
    ...
    def step1_initiate_signup(self) -> bool:
        ...
        self.log(f"  session_id={self.authorization_session_id}, action={self._action_id and self._action_id[:16]}...")
    ...
    def step4_get_password_page(self) -> bool:
        ...
        r = self.s.get(url, headers=self._get_headers(referer=f"{AUTH_BASE}/sign-up"), allow_redirects=True)
    ...
    def step6_get_email_verification_page(self) -> bool:
        ...
        r = self.s.get(url, headers=self._get_headers(referer=f"{AUTH_BASE}/sign-up/password"), allow_redirects=True)
        self.log(f"  -> {r.status_code}")
        action = self._extract_action_id(r.text)
        if action:
            self._action_id = action
    ...
    def step7_submit_otp(self, email: str, code: str, pending_auth_token: str) -> str:
        ...
        resp = self.s.post(url, data=body, headers={
            "accept": "text/x-component",
            "content-type": ct,
            "origin": AUTH_BASE,
            "referer": url,
    ...
    def step8_exchange_callback(self, auth_code: str) -> str:
        ...
        r = self.s.get(url, headers=self._get_headers(referer=AUTH_BASE), allow_redirects=True)
    ...
    def register(
        ...
        pending_token = self.step5_submit_password(email, password, first_name, last_name)
        if pending_token is None:
            return {"success": False, "error": "submit_password failed (email may already be registered)"}
        ...
        if not session_token:
            return {"success": False, "error": "exchange_callback failed / no wos-session"}
